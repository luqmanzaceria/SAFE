...
def _process_hidden_states(h) -> torch.Tensor:
    """Normalize hidden states to (T, D) regardless of saved format."""
...
def _pad_collate(batch):
    """Pad a list of {features, label} dicts to the same length."""
...
def _compute_loss(scores, masks, labels,
    ...
    """
    Margin-style loss:
      • for successful episodes → push scores toward 0
      • for failed episodes     → push scores toward 1
    Class-frequency re-weighting so imbalanced datasets don't bias the detector.
    """
    ...
    if model is not None and lambda_reg > 0:
        reg = sum(p.pow(2).sum() for n, p in model.named_parameters()
                  if "weight" in n)
...
def _interp_to_n(scores, n=100):
    ...
    return np.interp(np.linspace(0, len(scores) - 1, n),
...
def plot_per_task_auc(rollouts, scores, save_path):
    ...
    for tid in task_ids:
        mask  = [i for i, r in enumerate(rollouts) if r.task_id == tid]
        yt    = np.array([1 - rollouts[i].episode_success for i in mask])
        ys    = np.array([scores[i][-1] for i in mask])
        name  = rollouts[mask[0]].task_description[:40]
...
def _task_context(rollout, all_rollouts, all_scores, n_interp=80):
    """Return (mean_success, mean_fail) score curves for rollout's task."""
    tid = rollout.task_id
    succ, fail = [], []
    for r, s in zip(all_rollouts, all_scores):
        if r.task_id != tid:
            continue
        (succ if r.episode_success else fail).append(
            _interp_to_n(s, n_interp))
    xs = np.linspace(0, 1, n_interp)
    mean_s = np.stack(succ).mean(0) if succ else None
    std_s  = np.stack(succ).std(0)  if succ else None
    mean_f = np.stack(fail).mean(0) if fail else None
    std_f  = np.stack(fail).std(0)  if fail else None
    return xs, mean_s, std_s, mean_f, std_f
...
def make_annotated_video(rollout, scores, save_path,
    ...
    Side-by-side annotated MP4:
      Left  – RGB observation with a coloured border
              (green = safe, red = failure score > 0.5)
    ...
    xs_ctx, m_s, std_s, m_f, std_f = (None,) * 5
    ...
    for i, frame in enumerate(frames):
        ...
        status_color = "red" if alert else "green"
        axes[0].set_title(f"Frame {i}    {status}",
                          color=status_color, fontweight="bold", fontsize=10)

        # ── Right: score plot ──────────────────────────────────────────────
        ax = axes[1]

        # Task-level context ribbons
        ...
        # Current rollout up to now (also normalised)
...
def plot_summary(train_r, test_r, train_s, test_s, save_path):
    """
    4-panel overview:
      top-left:  score curves (train)   top-right: score curves (test)
      bot-left:  histogram              bot-right: PCA
    """
    ...
    def _draw_score_curves(ax, rollouts, scores, title):
    ...
    _draw_score_curves(fig.add_subplot(gs[0, 0]), train_r, train_s, "Score Curves — Train")
    _draw_score_curves(fig.add_subplot(gs[0, 1]), test_r,  test_s,  "Score Curves — Test")
    ...
    if succ_f:
        ax_hist.hist(succ_f, bins=bins, color="seagreen", alpha=0.6,
                     label=f"Success (n={len(succ_f)})", density=True)
    if fail_f:
        ax_hist.hist(fail_f, bins=bins, color="crimson",  alpha=0.6,
                     label=f"Failure (n={len(fail_f)})",  density=True)
    ax_hist.axvline(0.5, color="orange", linestyle="--", lw=1.5)
    ax_hist.set_xlabel("Final failure score"); ax_hist.set_ylabel("Density")
    ax_hist.set_title("Final Score Distribution — Test")
    ax_hist.legend(fontsize=8); ax_hist.grid(True, alpha=0.3)

    # PCA
    all_r = train_r + test_r
    all_s_arr = train_s + test_s
    ...
    for val, color, name in [(1, "seagreen", "Success"), (0, "crimson", "Failure")]:
        m = labels == val
        ax_pca.scatter(proj[m & (split == "train"), 0],
                       proj[m & (split == "train"), 1],
                       c=color, marker="o", alpha=0.5, s=18,
                       edgecolors="none", label=f"{name} (train)")
        ax_pca.scatter(proj[m & (split == "test"), 0],
                       proj[m & (split == "test"), 1],
                       c=color, marker="^", alpha=0.8, s=28,
                       edgecolors="none", label=f"{name} (test)")
    ax_pca.set_xlabel(f"PC 1  ({pca.explained_variance_ratio_[0]*100:.1f}%)")
    ax_pca.set_ylabel(f"PC 2  ({pca.explained_variance_ratio_[1]*100:.1f}%)")
    ax_pca.set_title("PCA of Mean Hidden States")
    ax_pca.legend(fontsize=7); ax_pca.grid(True, alpha=0.3)
...
def main():
    ...
    parser.add_argument("--data_path", required=True,
                        help="Directory containing task*--ep*--succ*.csv rollout files")
    ...
    parser.add_argument("--n_videos",    type=int,   default=10,
                        help="Max annotated videos to generate (half success, half failure)")
    ...
    # Stratified split: keep same success-rate in train and test
    rng = np.random.RandomState(args.seed)
    success_idx = [i for i, r in enumerate(all_rollouts) if r.episode_success]
    fail_idx    = [i for i, r in enumerate(all_rollouts) if not r.episode_success]
    rng.shuffle(success_idx); rng.shuffle(fail_idx)

    n_s_train = max(1, int(len(success_idx) * args.train_ratio))
    n_f_train = max(1, int(len(fail_idx)    * args.train_ratio))
    train_idx = success_idx[:n_s_train] + fail_idx[:n_f_train]
    test_idx  = success_idx[n_s_train:] + fail_idx[n_f_train:]
    ...
    if args.save_videos:
        ...
        for idx in tqdm(to_render, desc="Videos"):
            ...
            score_final = test_scores[idx][-1]
            fname = (f"task{r.task_id}_ep{r.episode_idx}"
                     f"_gt{tag}_score{score_final:.2f}.mp4")
        ...
        print("\n[5/5] Skipping videos  (pass --save_videos to enable)")

    # ── Summary stats ──────────────────────────────────────────────────────
    print("\n" + "=" * 60)
    print("RESULTS SUMMARY")
    print("=" * 60)
    if len(test_rollouts) > 0:
        y_true  = np.array([1 - r.episode_success for r in test_rollouts])
        y_score = np.array([s[-1] for s in test_scores])
        if len(np.unique(y_true)) > 1:
            auc_score = roc_auc_score(y_true, y_score)
            print(f"  Test AUC (failure detection):  {auc_score:.4f}")
        acc_thresh = np.mean((y_score > 0.5) == y_true)
        print(f"  Accuracy @ threshold 0.5:      {acc_thresh:.4f}")
